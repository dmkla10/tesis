...
model {
  ...
  for (ii in 1:I) {
    beta[ii] ~ normal(dot_product(theta01 + x[ii],theta1), 5);
    lambda[ii] ~ normal(dot_product(theta02 + x[ii],theta2), 5);
    alpha[ii] ~ normal(dot_product(theta02 + x[ii],theta2), 5);
  }
  for (n in 1:N) {
    ...
    y[n] ~ bernoulli_logit(link_preference + node_propension + group_preference + alpha[i[n]]);
  }
}