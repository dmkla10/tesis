...
Para representar matricialmente un hipergrafo, se utiliza una \textit{matríz de incidencia} $X \in \mathbb{R}^{n \times m}$, donde cada fila representa un vector, cada columna un hiperenlace. En esta notación, el valor de $H_{j,i}$ es 1 si el nodo $i$ pertenece al hipernlace $j$, y 0 si no.\\
...
Para apoyar esta postura, realizan un experimento donde comparan la 

En términos más informales, la predicción de hiperenlaces, como indica su nombre, debería \textit{predecir} los hiperenlaces no observados o futuros, o sea, debería entregar un conjunto de hiperenlaces predichos, cuando en cambio con la clasificación, lo que hacemos es mostrar al algoritmo un conjunto de hiperenlaces, y el algoritmo nos dice cuáles hiperenlaces de ese conjunto son los más probables de ser reales.\\

Como alternativa, en \cite{Yu_2024} se propone el siguiente planteamiento:

\begin{itemize}
    \item \textbf{Dado} (a) un hipergrafo $H = (V,E,W)$, con W la matriz de atributos de los nodos, (b) un conjunto de nodos de consulta $Q \subset V$, (c) un tamaño objetivo $s$, y (d) una cantidad de soluciones objetivo $k$
    \item \textbf{Encontrar} hasta $k$ hiperenlaces que pertenezcan al conjunto de hiperenlaces verdaderos positivos para nuestro hipergrafo
\end{itemize}

, \textbf{sujeto a} que el conjunto de nodos de consulta estén dentro de los hiperenlaces propuestos. (este parrafo está casi igual al del paper citado, está bien?)\\ 
